defmodule NodeTest do
  ...
  defp start_node(port, id \\ "") do
    ...
    spec = %{
      ...
      start: {ChordNode, :start_link, [%{id: nid, addr: addr}]}
    }
  ...
  setup do
    ...
    %{
      node: nid,
      node_details: %CNode{id: Utils.generate_hash("1"), address: "http://localhost:#{port}"},
      port: port
    }
  end
  ...
  test "node successor is itself", %{node: pid, node_details: cnode} do
    assert GenServer.call(pid, {:find_successor, cnode, 0}) == {cnode, 0}
  end

  test "node successor is itself if it\'s ID larger than other node", %{
    node_details: cnode,
    port: port
  } do
    ...
    assert GenServer.call(new_node, {:find_successor, cnode2, 0}) == {cnode2, 0}
  end
  ...
  test "node successor is node with larger ID", %{node: pid, node_details: cnode, port: port} do
    ...
    assert val == {%CNode{id: nid, address: cnode.address}, 0}
  end
